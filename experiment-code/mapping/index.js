...
app.get('/getTrials', function(request, response){  
    ...
    let subset = all_trial_content.filter(item => (!counter[item.img_id] || counter[item.img_id]<max_repeat_trial));
    console.log(subset, subset.length)
    if (subset.length>=max_num_unique_trial){
        response.json(subset);
    } else {
        response.status(500).send({'error': 'not enough unique trials!'});
    } 
})
